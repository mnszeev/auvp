Zeev.Controller = {
    Settings: {
        MapFields: () => {
            ...
            Zeev.Form.Functions.AddGrouping("Variáveis", "Variaveis", ["totalKmPercorrida", "totalDiasADescontar", "totalTarifa"]);
        },
        MapDataSources: () => {
        ...
        Init: async () => {
            Zeev.Resources.CustomVariables.Environments = "dev";
    ...
    CustomerRules: {
        FetchPostRequest: async (dataSource, params) => {
            let result = null
            await Zeev.Integration.Functions.ExecuteDataSource(Zeev.Integration.DataSources[dataSource], params, "POST")
                .then(response => { if (response) result = response; });
            return result;
        },
        FetchGetRequest: async (dataSource) => {
            let result = null
            await Zeev.Integration.Functions.ExecuteDataSource(Zeev.Integration.DataSources[dataSource], "GET")
                .then(response => { if (response) result = response; });
            return result;
        },
        GetCitiesOfRS: async () => {
        ...
        GetVehicleCompensationByReferenceMonth: async () => {
            ...
            const params = { inpmesReferencia: referenceMonth }
            const vehicleCompensationByReferenceMonth = await Zeev.Controller.CustomerRules.FetchPostRequest("VehicleCompensationByReferenceMonth", params);
            return [].concat(vehicleCompensationByReferenceMonth || []);
        },
        GenerateCsv: async () => {
            const citiesOfRS = await Zeev.Controller.CustomerRules.GetCitiesOfRS();
            const vehicleCompensationByReferenceMonth = await Zeev.Controller.CustomerRules.GetVehicleCompensationByReferenceMonth();
        ...
        FilterByArea: (servantList, area) => {
            if (area === "Servidores") {
                return servantList = servantList.filter(item =>
                    item.fields.flowResult === 'Enviado para aprovação de lote'
                );
            } else {
                return servantList = servantList.filter(item =>
                    item.fields.area === area && item.fields.flowResult === 'Enviado para aprovação de lote'
                );
            }
        },
        GenerateCsvFile: (area, servantList, cityList) => {
            ...
            for (const servant of filteredServantList) {
                ...
                const cityName = servant.fields.minicipio;
